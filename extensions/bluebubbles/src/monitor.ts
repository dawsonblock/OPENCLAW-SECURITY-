...
function readWebhookTokenHeader(req: IncomingMessage): string | undefined {
  ...
  const explicitHeader = req.headers["x-webhook-token"];
  const explicitValue = Array.isArray(explicitHeader) ? explicitHeader[0] : explicitHeader;
...
export async function handleBlueBubblesWebhookRequest(
  ...
  const matching = targets.filter((target) => {
    ...
    const providedToken = readWebhookTokenHeader(req);
    if (!providedToken) {
      return false;
    }
    if (constantTimeTokenEquals(token, providedToken)) {
      return true;
    }
    return false;
  });
  ...
  if (matching.length === 0) {
    res.statusCode = 401;
    res.setHeader("WWW-Authenticate", 'Bearer realm="openclaw-bluebubbles"');
    res.end("unauthorized");
    console.warn("[bluebubbles] webhook rejected: unauthorized token");
    return true;
  }
