...
- This plugin integrates Lobster with AetherBot _without core changes_.
...
## Using `openclaw.invoke` (Lobster → AetherBot tools)

Some Lobster pipelines may include a `openclaw.invoke` step to call back into AetherBot tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).

For this to work, the AetherBot Gateway must expose the tool bridge endpoint and the target tool must be allowed by policy:

- AetherBot provides an HTTP endpoint: `POST /tools/invoke`.
- The request is gated by **gateway auth** (e.g. `Authorization: Bearer …` when token auth is enabled).
- The invoked tool is gated by **tool policy** (global + per-agent + provider + group policy). If the tool is not allowed, AetherBot returns `404 Tool not available`.
