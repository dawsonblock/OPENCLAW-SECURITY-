...
function checkSwiftCLIIntegration() {
  ...
  if (
    !invalidWindowOutput.includes("invalid for") ||
    !invalidWindowOutput.includes("window-index")
  ) {
  ...
  const formats = [{ cmd: "./peekaboo list apps --json-output", required: ["success", "data"] }];
...
function checkVersionConsistency() {
  ...
  if (packageVersion !== versionFileVersion) {
    logError(
      `Version mismatch: package.json has ${packageVersion}, version.json has ${versionFileVersion}`,
    );
    return false;
  }
...
function buildAndVerifyPackage() {
  ...
  const buildScript = requireUniversal
    ? "./scripts/build-swift-universal.sh"
    : "./scripts/build-swift-arm.sh";
  ...
  const requiredFiles = ["peekaboo", "peekaboo-mcp.js", "README.md", "LICENSE"];
