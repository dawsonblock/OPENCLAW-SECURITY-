    ...
    <div className="flex flex-col h-screen max-w-2xl mx-auto p-4">
      <div className="flex-1 overflow-y-auto space-y-4 mb-4">
        {messages.map((message) => (
          ...
          >
            <p className="font-semibold">{message.role === "user" ? "You" : "Claude 4"}</p>
            {message.parts.map((part, index) => {
              ...
              if (part.type === "reasoning") {
                return (
                  <pre key={index} className="bg-gray-100 p-2 rounded mt-2 text-xs overflow-x-auto">
                    <details>
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit}>
        <input name="prompt" value={input} onChange={(e) => setInput(e.target.value)} />
...
const { text: answer } = await generateText({
  model: deepinfra("meta-llama/Meta-Llama-3.1-70B-Instruct"),
  system: "You are solving math problems. Reason step by step. Use the calculator when necessary.",
  prompt: problem,
  tools: {
    calculate: tool({
    ...
    >({
      transform(chunk, controller) {
        switch (chunk.type) {
          ...
          case "text-end": {
            console.log(`Text block ${chunk.id} completed:`, textBlocks.get(chunk.id));
            break;
          }
        }
            ...
            switch (part.type) {
              ...
              case "tool-getWeatherInformation": {
                ...
                switch (part.state) {
                  ...
                  case "input-available":
                    return (
                      <div key={callId}>Getting weather information for {part.input.city}...</div>
                    );
                  case "output-available":
                    return (
                      <div key={callId}>
...
async function resumeConversation(lastMessage: UIMessage) {
  const result = streamText({
    model: openai("gpt-4o"),
    messages: [{ role: "user", content: "Continue our previous conversation." }],
  });
      ...
      {messages.map((message) => (
        <div key={message.id}>
          ...
          {message.metadata?.totalTokens && (
            <div className="text-xs text-gray-400">{message.metadata.totalTokens} tokens</div>
          )}
        </div>
      ))}
    </div>
...
console.log(`Revised prompt: ${providerMetadata.vertex.images[0].revisedPrompt}`);
```
