...
- **Risky suites**: see the table below—anything marked _High_ should only run on a disposable VM snapshot.
...
Only enable the _High_-risk suites when you’re inside a dedicated VM snapshot and expect the UI to shift. Leave `RUN_AGENT_TESTS` unset unless you specifically want to exercise agent-driven flows.
