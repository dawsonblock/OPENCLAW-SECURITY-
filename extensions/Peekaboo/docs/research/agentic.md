...
- Injected as **two messages**:
  - **System policy** message: declares `DESKTOP_STATE` as _untrusted data_; never instructions.
...
- Window titles / clipboard contents are classic _indirect prompt injection_ vectors.
- Keep “policy” stable and high-priority (system).
- Keep _untrusted content_ out of system/developer tiers (data is user-role), while still providing provenance signals (delimiters + datamarking).
...
Peekaboo is an _agentic_ system with:
...
- For each tool call:
  ...
  - if `enhancementOptions.verifyActions == true` and tool is mutating:
    - capture _after-action_ screenshot (prefer region around action point if available),
