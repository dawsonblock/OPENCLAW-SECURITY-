...
describe("gateway node command allowlist", () => {
  test("enforces command allowlists across node clients", async () => {
    ...
    let approvalsClient: GatewayClient | undefined;
    ...
    try {
      ...
      approvalsClient = await connectNodeClient({
        port,
        commands: ["system.execApprovals.get", "system.execApprovals.set"],
        instanceId: "node-exec-approvals",
        displayName: "node-exec-approvals",
      });
      const approvalsNodeId = await getConnectedNodeId();
      const approvalsGetRes = await rpcReq(ws, "exec.approvals.node.get", {
        nodeId: approvalsNodeId,
      });
      expect(approvalsGetRes.ok).toBe(false);
      expect(approvalsGetRes.error?.message).toContain("node command not allowed");
      const approvalsSetRes = await rpcReq(ws, "exec.approvals.node.set", {
        nodeId: approvalsNodeId,
        file: { version: 1 },
      });
