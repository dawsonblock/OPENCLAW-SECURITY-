...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.aetherbot.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + AetherBot hooks")
    ...
    .option("--hook-url <url>", "AetherBot hook URL")
    .option("--hook-token <token>", "AetherBot hook token")
