...
export function registerAgentCommands(program: Command, args: { agentChannelOptions: string }) {
  program
    ...
    .option("--replay", "Run in deterministic replay mode (implies --local)", false)
    .option(
      "--timeout <seconds>",
...
${formatHelpExamples([
  ['aetherbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['aetherbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'aetherbot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'aetherbot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['aetherbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'aetherbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.aetherbot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.aetherbot.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'aetherbot agents set-identity --agent main --name "AetherBot" --emoji "🦞"',
    "Set name + emoji.",
  ],
  ["aetherbot agents set-identity --agent main --avatar avatars/aetherbot.png", "Set avatar path."],
  [
    "aetherbot agents set-identity --workspace ~/.aetherbot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "aetherbot agents set-identity --identity-file ~/.aetherbot/workspace/IDENTITY.md --agent main",
