...
export function registerPairingCli(program: Command) {
  ...
  const pairing = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/pairing", "docs.aetherbot.ai/cli/pairing")}\n`,
    );
  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      if (!opts.channel && !code) {
        throw new Error(
          `Usage: ${formatCliCommand("aetherbot pairing approve <channel> <code>")} (or: ${formatCliCommand("aetherbot pairing approve --channel <channel> <code>")})`,
        );
      }
      if (opts.channel && code != null) {
        throw new Error(
          `Too many arguments. Use: ${formatCliCommand("aetherbot pairing approve --channel <channel> <code>")}`,
        );
      }
      const channel = parseChannel(channelRaw, channels);
