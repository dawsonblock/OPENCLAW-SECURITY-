...
const STEP_LABELS: Record<string, string> = {
  ...
  "aetherbot doctor entry": "Checking doctor entrypoint",
  "aetherbot doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "aetherbot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const AETHERBOT_REPO_URL = "https://github.com/AetheronResearch/aetherbot.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("aetherbot@") || trimmed.startsWith("openclaw@")) {
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "aetherbot.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("aetherbot completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AETHERBOT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENCLAW_GIT_DIR to an empty folder or an aetherbot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("AetherBot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating AetherBot..."));
    defaultRuntime.log("");
  }
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("aetherbot gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aetherbot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("aetherbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aetherbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `aetherbot update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OPENCLAW_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENCLAW_GIT_DIR to an empty folder or an aetherbot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AetherBot to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["aetherbot update", "Update a source checkout (git)"],
        ["aetherbot update --channel beta", "Switch to beta channel (git + npm)"],
        ["aetherbot update --channel dev", "Switch to dev channel (git + npm)"],
        ["aetherbot update --tag beta", "One-off update to a dist-tag or version"],
        ["aetherbot update --no-restart", "Update without restarting the service"],
        ["aetherbot update --json", "Output result as JSON"],
        ["aetherbot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["aetherbot update wizard", "Interactive update wizard"],
        ["aetherbot --update", "Shorthand for aetherbot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run aetherbot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.aetherbot.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aetherbot update status", "Show channel + version status."],
          ["aetherbot update status --json", "JSON output."],
          ["aetherbot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.aetherbot.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
