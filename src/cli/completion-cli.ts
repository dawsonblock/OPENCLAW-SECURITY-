...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# AetherBot Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# AetherBot Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(aetherbot completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "aetherbot") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
