...
const consumedEntries = new Set<string>(); // Set of Ledger Entry Hashes or IDs?
// Ledger entries don't have unique IDs across sessions unless we use hashes.
// But we have the full entry list in memory. We can just track index?
// Or we can add a transient property to the cached objects.
// Since `replayCache` stores object references, we can mutate them with a symbol.
    ...
    execute: async (toolCallId, args, signal, onUpdate) => {
      ...
      if (isReplayMode()) {
        ...
        if (match) {
          ...
          return match.output as AgentToolResult<unknown>;
        } else {
          log.warn(
      ...
      if (!isReplayMode()) {
        try {
          ...
          log.warn(`Ledger: Failed to record proposal: ${e}`);
        }
      }
      ...
      try {
        ...
        if (!isReplayMode()) {
          ...
          try {
            ...
            log.warn(`Ledger: Failed to record result: ${e}`);
          }
        }
      }
