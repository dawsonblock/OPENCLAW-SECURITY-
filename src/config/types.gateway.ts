...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "aetherbot.internal"). */
  domain?: string;
};
...
export type CanvasHostConfig = {
  enabled?: boolean;
  /** Directory to serve (default: ~/.aetherbot/workspace/canvas). */
  root?: string;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/aetherbot"). */
  basePath?: string;
...
export type GatewayConfig = {
   ...
   * - custom: User-specified IP, fallback to loopback if unavailable (requires customBindHost)
   * Default: loopback (127.0.0.1).
   */
  bind?: GatewayBindMode;
  /** Custom IP address for bind="custom" mode. Fallback: loopback. */
