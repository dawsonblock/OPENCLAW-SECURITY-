...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*aetherbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "aetherbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "aetherbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "aetherbot-gw");
  ...
  records.push(`_aetherbot-gw._tcp IN PTR ${instanceLabel}._aetherbot-gw._tcp`);
  records.push(`${instanceLabel}._aetherbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._aetherbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; aetherbot-content-hash: ${contentHash}\n${contentBody}`;
}
