...
const SEARCH_TOOL = "https://docs.aetherbot.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.aetherbot.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('aetherbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.aetherbot.ai/");
      runtime.log(`Search: ${formatCliCommand('aetherbot docs "your query"')}`);
    }
    return;
  }
