...
This directory contains hooks that ship with AetherBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.aetherbot/workspace`)
...
**Output**: `~/.aetherbot/logs/commands.log`
...
**Docs**: https://docs.aetherbot.ai/hooks/soul-evil
...
metadata:
  { "aetherbot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.aetherbot/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.aetherbot/aetherbot.json`:
...
2. Restart gateway: `pkill -9 -f 'aetherbot.*gateway' && pnpm aetherbot gateway`
3. Enable the hook: `aetherbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.aetherbot.ai/hooks
