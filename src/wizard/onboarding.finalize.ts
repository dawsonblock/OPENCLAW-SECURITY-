...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.aetherbot.ai/gateway/health",
          "https://docs.aetherbot.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.aetherbot.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    if (hatchChoice === "tui") {
      ...
      if (browserSupport.ok) {
        controlUiOpened = await openUrl(controlUiLaunchUrl);
        if (!controlUiOpened) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.aetherbot.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.aetherbot.ai/security",
    "Security",
  );
