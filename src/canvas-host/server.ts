...
(() => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AetherBot node canvas.");
      return;
    }
...
function isLoopbackCanvasHost(hostRaw: string): boolean {
  const host = hostRaw
    .trim()
    .toLowerCase()
    .replace(/^\[|\]$/g, "");
...
function readBearerToken(rawAuth: string | string[] | undefined): string | null {
...
function isCanvasRequestAuthorized(
  req: IncomingMessage,
  opts: { requiredToken?: string },
): boolean {
  const expected = opts.requiredToken?.trim();
  if (!expected) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>AetherBot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
...
export async function startCanvasHost(opts: CanvasHostServerOpts): Promise<CanvasHostServer> {
  ...
  const allowLan = isTruthyEnvValue(process.env.OPENCLAW_CANVAS_ALLOW_LAN);
  const requiredToken = opts.authToken?.trim() || process.env.OPENCLAW_CANVAS_TOKEN?.trim();
  const loopbackOnly = isLoopbackCanvasHost(bindHost);

  if (!loopbackOnly && !allowLan) {
    throw new Error(`refusing canvas host bind to ${bindHost} without OPENCLAW_CANVAS_ALLOW_LAN=1`);
  }
  if (!loopbackOnly && !requiredToken) {
    throw new Error(
      `refusing canvas host bind to ${bindHost} without OPENCLAW_CANVAS_TOKEN or canvasHost.authToken`,
    );
  }
  if (!loopbackOnly) {
  ...
  const server: Server = http.createServer((req, res) => {
    ...
    void (async () => {
      if (!isCanvasRequestAuthorized(req, { requiredToken })) {
        res.statusCode = 401;
        res.setHeader("WWW-Authenticate", 'Bearer realm="openclaw-canvas"');
        res.setHeader("Content-Type", "text/plain; charset=utf-8");
  ...
  server.on("upgrade", (req, socket, head) => {
    if (!isCanvasRequestAuthorized(req, { requiredToken })) {
      ...
      socket.write('WWW-Authenticate: Bearer realm="openclaw-canvas"\r\n');
