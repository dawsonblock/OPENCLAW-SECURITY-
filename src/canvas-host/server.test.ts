...
describe("canvas host", () => {
  ...
  it("refuses LAN bind unless explicitly enabled", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-canvas-"));
    const prevAllowLan = process.env.OPENCLAW_CANVAS_ALLOW_LAN;

    try {
      delete process.env.OPENCLAW_CANVAS_ALLOW_LAN;
      await expect(
        startCanvasHost({
          runtime: defaultRuntime,
          rootDir: dir,
      ...
      ).rejects.toThrow("OPENCLAW_CANVAS_ALLOW_LAN");
    } finally {
      if (typeof prevAllowLan === "string") {
        process.env.OPENCLAW_CANVAS_ALLOW_LAN = prevAllowLan;
      } else {
        delete process.env.OPENCLAW_CANVAS_ALLOW_LAN;
      }
      await fs.rm(dir, { recursive: true, force: true });
  ...
  it("requires a token when LAN bind is enabled", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-canvas-"));
    const prevAllowLan = process.env.OPENCLAW_CANVAS_ALLOW_LAN;
    const prevToken = process.env.OPENCLAW_CANVAS_TOKEN;

    try {
      process.env.OPENCLAW_CANVAS_ALLOW_LAN = "1";
      delete process.env.OPENCLAW_CANVAS_TOKEN;
      await expect(
        startCanvasHost({
          runtime: defaultRuntime,
          rootDir: dir,
      ...
      ).rejects.toThrow("OPENCLAW_CANVAS_TOKEN");
    } finally {
      if (typeof prevAllowLan === "string") {
        process.env.OPENCLAW_CANVAS_ALLOW_LAN = prevAllowLan;
      } else {
      ...
      if (typeof prevToken === "string") {
        process.env.OPENCLAW_CANVAS_TOKEN = prevToken;
      } else {
        delete process.env.OPENCLAW_CANVAS_TOKEN;
      }
      await fs.rm(dir, { recursive: true, force: true });
  ...
  it("enforces bearer auth when authToken is configured", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-canvas-"));
    await fs.writeFile(path.join(dir, "index.html"), "<html><body>secure</body></html>", "utf8");
    const server = await startCanvasHost({
      runtime: defaultRuntime,
      rootDir: dir,
      port: 0,
    ...
    try {
      const unauthorized = await fetch(`http://127.0.0.1:${server.port}${CANVAS_HOST_PATH}/`);
      expect(unauthorized.status).toBe(401);

      const authorized = await fetch(`http://127.0.0.1:${server.port}${CANVAS_HOST_PATH}/`, {
        headers: { Authorization: "Bearer canvas-secret" },
      });
      expect(authorized.status).toBe(200);
      expect(await authorized.text()).toContain("secure");
    } finally {
      await server.close();
      await fs.rm(dir, { recursive: true, force: true });
    }
