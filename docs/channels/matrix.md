...
Matrix is an open, decentralized messaging protocol. AetherBot connects as a Matrix **user**
...
```bash
aetherbot plugins install @openclaw/matrix
```

Local checkout (when running from a git repo):

```bash
aetherbot plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `aetherbot plugins install @openclaw/matrix`
   - From a local checkout: `aetherbot plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AetherBot calls the same
...
- On first connection, AetherBot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AetherBot logs a warning.
...
- Approve via:
  - `aetherbot pairing list matrix`
  - `aetherbot pairing approve matrix <CODE>`
...
- On startup, AetherBot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
