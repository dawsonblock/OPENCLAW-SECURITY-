...
Tlon is a decentralized messenger built on Urbit. AetherBot connects to your Urbit ship and can
...
```bash
aetherbot plugins install @openclaw/tlon
```

Local checkout (when running from a git repo):

```bash
aetherbot plugins install ./extensions/tlon
```
...
Use these with `aetherbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AetherBot replies in-thread.
