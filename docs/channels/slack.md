...
3. Set tokens for AetherBot and start the gateway.
...
AetherBot can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, AetherBot falls back to the user token.
...
```json
{
  "display_information": {
    "name": "AetherBot",
    "description": "Slack connector for AetherBot"
  },
  "features": {
    "bot_user": {
      "display_name": "AetherBot",
      "always_online": false
    },
    "app_home": {
...
By default, AetherBot replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, AetherBot infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `aetherbot pairing approve slack <code>`.
...
- On startup, AetherBot resolves channel/user names in allowlists to IDs (when tokens allow)
