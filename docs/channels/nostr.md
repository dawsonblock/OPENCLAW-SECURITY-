...
read_when:
  - You want AetherBot to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables AetherBot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`aetherbot onboard`) and `aetherbot channels add` list optional channel plugins.
...
```bash
aetherbot plugins install @openclaw/nostr
```

Use a local checkout (dev workflows):

```bash
aetherbot plugins install --link <path-to-openclaw>/extensions/nostr
```
