...
AetherBot can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, AetherBot retries the same message as plain text.

## Commands (native + custom)

AetherBot registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; AetherBot does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `aetherbot logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). AetherBot can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. AetherBot:
...
Private chats can include `message_thread_id` in some edge cases. AetherBot keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2. Run `aetherbot logs --follow` and look for `from.id`.
...
AetherBot defaults to audio files for backward compatibility.
...
AetherBot defaults to video files.
...
AetherBot supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, AetherBot handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, AetherBot caches these descriptions to avoid redundant API calls.
...
AetherBot uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, AetherBot:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by AetherBot)
...
- Example: `aetherbot message send --channel telegram --target 123456789 --message "hi"`.
...
- `aetherbot channels status` shows a warning when config expects unmentioned group messages.
- `aetherbot channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `aetherbot logs --follow` (look for "skipping group message")
...
- Upgrade to a AetherBot build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
