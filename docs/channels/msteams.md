...
```bash
aetherbot plugins install @openclaw/msteams
```

Local checkout (when running from a git repo):

```bash
aetherbot plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
AetherBot will offer the local install path automatically.
...
3. Configure AetherBot with those credentials.
...
- Talk to AetherBot via Teams DMs, group chats, or channels.
...
- On startup, AetherBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring AetherBot, you need to create an Azure Bot resource.
...
1. **Install the Microsoft Teams plugin**
   - From npm: `aetherbot plugins install @openclaw/msteams`
   - From a local checkout: `aetherbot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "AetherBot in Teams", "full": "AetherBot in Teams" },
...
AetherBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, AetherBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/AetherBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

AetherBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `aetherbot message poll --channel msteams --target conversation:<id> ...`
...
```bash
aetherbot message send --channel msteams \
...
aetherbot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
aetherbot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
aetherbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
aetherbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
