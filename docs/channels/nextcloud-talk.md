...
```bash
aetherbot plugins install @openclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
aetherbot plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "AetherBot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `aetherbot pairing list nextcloud-talk`
  - `aetherbot pairing approve nextcloud-talk <CODE>`
