...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects AetherBot to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
aetherbot plugins install @openclaw/feishu
```

Local checkout (when running from a git repo):

```bash
aetherbot plugins install ./extensions/feishu
```
...
If you just installed AetherBot, run the wizard:
...
2. Configuring app credentials in AetherBot
...
1. You already ran `aetherbot channels add` for Feishu
2. The gateway is running (`aetherbot gateway status`)
...
```bash
aetherbot pairing approve feishu <CODE>
```
...
2. Run `aetherbot logs --follow` and look for `chat_id`
...
2. Run `aetherbot logs --follow` and look for `open_id`
...
| Command                     | Description                   |
| --------------------------- | ----------------------------- |
| `aetherbot gateway status`  | Show gateway status           |
| `aetherbot gateway install` | Install/start gateway service |
| `aetherbot gateway stop`    | Stop gateway service          |
| `aetherbot gateway restart` | Restart gateway service       |
| `aetherbot logs --follow`   | Tail gateway logs             |
...
4. Check logs: `aetherbot logs --follow`
...
5. Ensure the gateway is running: `aetherbot gateway status`
6. Check logs: `aetherbot logs --follow`
