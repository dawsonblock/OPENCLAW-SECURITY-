...
3. Run `aetherbot channels login` to scan the QR code (Linked Devices).
4. Start the gateway.
...
WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run AetherBot on WhatsApp:

### Dedicated number (recommended)

Use a **separate phone number** for AetherBot. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‑Fi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate — install WhatsApp Business and register the AetherBot number there.
...
Quick fallback: run AetherBot on **your own number**. Message yourself (WhatsApp “Message yourself”) for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
- Early AetherBot builds supported Twilio’s WhatsApp Business integration.
...
- Login command: `aetherbot channels login` (QR via Linked Devices).
- Multi-account login: `aetherbot channels login --account <id>` (`<id>` = `accountId`).
...
- Logout: `aetherbot channels logout` (or `--account <id>`) deletes WhatsApp auth state (but keeps shared `oauth.json`).
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `aetherbot pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run AetherBot on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will AetherBot message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. AetherBot only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:

- First DM from a new sender returns a short code (message is not processed).
- Approve with: `aetherbot pairing approve whatsapp <code>` (list with `aetherbot pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different AetherBot instances on one WhatsApp number?**  
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `aetherbot message send --media <mp4> --gif-playback`
...
- Best results: OGG/Opus. AetherBot rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Fix: run `aetherbot channels login` on the gateway host and scan the QR (WhatsApp → Settings → Linked Devices).

**Linked but disconnected / reconnect loop**

- Symptom: `channels status` shows `running, disconnected` or warns “Linked but disconnected”.
- Fix: `aetherbot doctor` (or restart the gateway). If it persists, relink via `channels login` and inspect `aetherbot logs --follow`.
