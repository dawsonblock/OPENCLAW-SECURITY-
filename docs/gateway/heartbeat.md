...
- During heartbeat runs, AetherBot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `aetherbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AetherBot skips the heartbeat run entirely (no model call).
...
```bash
aetherbot system event --text "Check for urgent follow-ups" --mode now
```
