...
AetherBot reads an optional **JSON5** config from `~/.openclaw/openclaw.json` (comments + trailing commas allowed).

If the file is missing, AetherBot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.openclaw/workspace`). You usually only need a config to:
...
AetherBot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `aetherbot doctor`, `aetherbot logs`, `aetherbot health`, `aetherbot status`, `aetherbot service`, `aetherbot help`).
- Run `aetherbot doctor` to see the exact issues.
- Run `aetherbot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `aetherbot config set`. Keep a backup of `~/.openclaw/openclaw.json`.
...
```bash
aetherbot gateway call config.get --params '{}' # capture payload.hash
aetherbot gateway call config.apply --params '{
...
```bash
aetherbot gateway call config.get --params '{}' # capture payload.hash
aetherbot gateway call config.patch --params '{
...
AetherBot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, AetherBot runs your login shell and imports only the missing expected keys (never overrides).
...
AetherBot stores **per-agent** auth profiles (OAuth + API keys) in:
...
- `~/.openclaw/agent/*` (migrated by `aetherbot doctor` into `~/.openclaw/agents/<defaultAgentId>/agent/*`)
...
On first use, AetherBot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, AetherBot derives defaults (only when you haven’t set them explicitly):
...
- The legacy single-account Baileys auth dir is migrated by `aetherbot doctor` into `whatsapp/default`.
...
AetherBot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
AetherBot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `aetherbot plugins install @openclaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
AetherBot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
AetherBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, AetherBot
...
When a file exceeds this limit, AetherBot logs a warning and injects a truncated
...
message envelopes). If unset, AetherBot uses the host timezone at runtime.
...
`"[openclaw]"`, AetherBot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, AetherBot generates audio
...
AetherBot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, AetherBot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `aetherbot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:openclaw`: all built-in AetherBot tools (excludes provider plugins)
...
AetherBot uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.openclaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
AetherBot config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, AetherBot writes/merges a `models.json` into
...
OpenCode Zen is a multi-model gateway with per-model endpoints. AetherBot uses
...
- Shortcut: `aetherbot onboard --auth-choice opencode-zen`.
...
Shortcut: `aetherbot onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `aetherbot onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `aetherbot onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `aetherbot onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `aetherbot onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `aetherbot onboard --auth-choice minimax-api`.
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, AetherBot stays in idle-only mode for backward compatibility.
...
### `browser` (aetherbot-managed browser)

AetherBot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for aetherbot and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (AetherBot.app menubar, or `aetherbot gateway`).
...
- When a connection comes from one of these IPs, AetherBot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `aetherbot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
...
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  ...
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. AetherBot
...
- AetherBot.app watches `~/.openclaw/openclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
- `aetherbot --dev …` → uses `~/.aetherbot-dev` + shifts ports from base `19001`
- `aetherbot --profile <name> …` → uses `~/.aetherbot-<name>` (port via config/env/flags)
...
OPENCLAW_STATE_DIR=~/.aetherbot-a \
aetherbot gateway --port 19001
```
...
Gmail helper config (used by `aetherbot webhooks gmail setup` / `run`):
...
Note: when `tailscale.mode` is on, AetherBot defaults `serve.path` to `/` so
...
Default port: `18793` (chosen to avoid the aetherbot browser CDP port `18792`)  
