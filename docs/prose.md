---
summary: "OpenProse: .prose workflows, slash commands, and state in AetherBot"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In AetherBot it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
aetherbot plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `aetherbot plugins install ./extensions/open-prose`

Related docs: [Plugins](/tools/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses AetherBot tools under the hood.
...
## AetherBot runtime mapping

OpenProse programs map to AetherBot primitives:

| OpenProse concept         | AetherBot tool   |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use AetherBot tool allowlists and approval gates to control side effects.
