...
AetherBot features that can generate provider usage or paid API calls.
...
- `aetherbot status --usage` and `aetherbot channels list` show provider **usage windows**
...
AetherBot can pick up credentials from:
...
`aetherbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
