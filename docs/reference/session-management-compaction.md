...
This document explains how AetherBot manages sessions end-to-end:
...
AetherBot is designed around a single **Gateway process** that owns session state.
...
AetherBot persists sessions in two layers:
...
AetherBot resolves these via `src/config/sessions.ts`.
...
AetherBot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (AetherBot consumes the events, but Pi decides when to compact).
...
AetherBot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, AetherBot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, AetherBot leaves it alone.
...
- `aetherbot status` (CLI)
- `aetherbot sessions` / `sessions --json`
...
AetherBot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- AetherBot strips/suppresses this in the delivery layer.

As of `2026.1.10`, AetherBot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
AetherBot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but AetherBot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `aetherbot status`.
