...
AetherBot integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- AetherBot owns lifecycle when `channels.signal.autoStart=true`.
...
- AetherBot spawns `imsg rpc` as a child process (legacy iMessage integration).
