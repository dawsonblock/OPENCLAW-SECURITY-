...
AetherBot has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
### Layer 2: Gateway + dev agent smoke (what “@aetherbot” actually does)
...
```bash
aetherbot models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: AetherBot calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: AetherBot shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `aetherbot models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `aetherbot models list` / model selection.
...
- `OPENCLAW_CONFIG_DIR=...` (default: `~/.aetherbot`) mounted to `/home/node/.openclaw`
