---
summary: "Gmail Pub/Sub push wired into AetherBot webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to AetherBot
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> AetherBot

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> AetherBot webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- AetherBot hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the AetherBot helper to wire everything together (installs deps on macOS via brew):

```bash
aetherbot webhooks gmail setup \
...
- Writes `hooks.gmail` config for `aetherbot webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, AetherBot automatically sets
...
- `--hook-url` points to AetherBot `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to AetherBot.

Recommended: `aetherbot webhooks gmail run` wraps the same flow and auto-renews the watch.
