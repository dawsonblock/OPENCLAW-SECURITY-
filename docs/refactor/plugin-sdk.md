...
Accessed via `AetherBotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AetherBotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AetherBotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AetherBotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AetherBotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AetherBotConfig): string;
  };
};
```
...
- Add `api.runtime` to `AetherBotPluginApi` with the surface above.
