...
read_when:
  ...
  - Deciding: standalone CLI vs deep AetherBot integration
...
Recommendation: **deep integration in AetherBot**, but keep a separable core library.

### Why integrate into AetherBot?

- AetherBot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `aetherbot memory recall "…" --k 25 --since 30d`
  - `aetherbot memory reflect --since 7d`
