...
AetherBot supports **camera capture** for agent workflows:
...
```bash
aetherbot nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
aetherbot nodes camera snap --node <id> --facing front
aetherbot nodes camera clip --node <id> --duration 3000
aetherbot nodes camera clip --node <id> --no-audio
```
...
```bash
aetherbot nodes camera list --node <id>            # list camera ids
aetherbot nodes camera snap --node <id>            # prints MEDIA:<path>
aetherbot nodes camera snap --node <id> --max-width 1280
aetherbot nodes camera snap --node <id> --delay-ms 2000
aetherbot nodes camera snap --node <id> --device-id <id>
aetherbot nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
aetherbot nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
aetherbot nodes camera clip --node <id> --device-id <id>
aetherbot nodes camera clip --node <id> --no-audio
```

Notes:

- `aetherbot nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
aetherbot nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
