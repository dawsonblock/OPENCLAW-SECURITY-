---
summary: "Use MiniMax M2.1 in AetherBot"
read_when:
  - You want MiniMax models in AetherBot
...
```bash
aetherbot plugins enable minimax-portal-auth  # skip if already loaded.
aetherbot gateway restart  # restart if gateway is already running
aetherbot onboard --auth-choice minimax-portal
```
...
- Use `aetherbot models list` and `aetherbot models set minimax/MiniMax-M2.1` to switch.
...
- Running `aetherbot configure` and selecting **MiniMax M2.1**, or
