---
summary: "Use Venice AI privacy-focused models in AetherBot"
read_when:
  - You want privacy-focused inference in AetherBot
...
```bash
aetherbot onboard --auth-choice venice-api-key
```
...
```bash
aetherbot onboard --non-interactive \
...
```bash
aetherbot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, AetherBot shows all available Venice models. Pick based on your needs:
...
```bash
aetherbot models set venice/claude-opus-45
aetherbot models set venice/llama-3.3-70b
```

List all available models:

```bash
aetherbot models list | grep venice
```

## Configure via `aetherbot configure`

1. Run `aetherbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AetherBot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
aetherbot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
aetherbot chat --model venice/claude-opus-45

# Use uncensored model
aetherbot chat --model venice/venice-uncensored

# Use vision model with image
aetherbot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
aetherbot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `aetherbot models list` to see currently available models. Some models may be temporarily offline.
