---
summary: "Use Synthetic's Anthropic-compatible API in AetherBot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. AetherBot registers it as the
...
```bash
aetherbot onboard --auth-choice synthetic-api-key
```
...
Note: AetherBot's Anthropic client appends `/v1` to the base URL, so use
