...
read_when:
  - You want to understand what “context” means in AetherBot
...
“Context” is **everything AetherBot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (AetherBot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How AetherBot builds the system prompt

The system prompt is **AetherBot-owned** and rebuilt each run. It includes:
...
By default, AetherBot injects a fixed set of workspace files (if present):
