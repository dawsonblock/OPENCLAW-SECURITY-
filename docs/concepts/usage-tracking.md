...
- `/usage cost` in chats: local cost summary aggregated from AetherBot session logs.
- CLI: `aetherbot status --usage` prints a full per-provider breakdown.
- CLI: `aetherbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
