---
title: Lobster
summary: "Typed workflow runtime for AetherBot with resumable approval gates."
description: Typed workflow runtime for AetherBot — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets AetherBot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: AetherBot runs one Lobster tool call and gets a structured result.
...
AetherBot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In AetherBot tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the AetherBot Gateway (see the [Lobster repo](https://github.com/openclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), AetherBot keeps core tools enabled. To restrict core
...
→ aetherbot calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ aetherbot calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls AetherBot tools that do.
