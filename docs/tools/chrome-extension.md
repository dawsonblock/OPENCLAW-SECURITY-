---
summary: "Chrome extension: let AetherBot drive your existing Chrome tab"
read_when:
...
The AetherBot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openclaw-managed Chrome profile.
...
AetherBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the AetherBot release (npm package) as static files. There is no separate “build” step.

After upgrading AetherBot:

- Re-run `aetherbot browser extension install` to refresh the installed files under your AetherBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

AetherBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want AetherBot to control.
...
- `ON`: attached; AetherBot can drive that tab.
...
Debugging: `aetherbot sandbox explain`
...
`aetherbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `aetherbot browser extension install` first to copy the extension to a stable location under your AetherBot state directory.
