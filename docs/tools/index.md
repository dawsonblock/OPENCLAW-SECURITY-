---
summary: "Agent tool surface for AetherBot (browser, canvas, nodes, message, cron) replacing legacy `openclaw-*` skills"
read_when:
...
AetherBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
...
- If `tools.allow` only references unknown or unloaded plugin tool names, AetherBot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`aetherbot node run`).
...
- Requires a Brave API key (recommended: `aetherbot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated AetherBot-managed browser.
...
- Quick smoke: `aetherbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `aetherbot gateway` restart in-place)
