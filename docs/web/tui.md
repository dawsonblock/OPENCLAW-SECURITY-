...
```bash
aetherbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `aetherbot tui --deliver`
...
- Check the Gateway logs: `aetherbot logs --follow`.
- Confirm the agent can run: `aetherbot status` and `aetherbot models status`.
...
- No agents in picker: check `aetherbot agents list` and your routing config.
