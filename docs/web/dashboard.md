...
- Re-open anytime: `aetherbot dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- Ensure the gateway is reachable (local: `aetherbot status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
- Retrieve the token from the gateway host: `aetherbot config get gateway.auth.token` (or generate one: `aetherbot doctor --generate-gateway-token`).
