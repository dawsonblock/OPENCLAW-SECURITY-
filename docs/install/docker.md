---
summary: "Optional Docker-based setup and onboarding for AetherBot"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run AetherBot on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full AetherBot in Docker)
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  AetherBot auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `aetherbot sandbox recreate ...` command.
...
- Custom tools not found: AetherBot runs commands with `sh -lc` (login shell), which
