...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AetherBot
...
Running AetherBot via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `aetherbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch aetherbot.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node aetherbot.mjs status` works on Node 25.
