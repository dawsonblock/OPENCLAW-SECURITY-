---
summary: "macOS app flow for controlling a remote AetherBot gateway over SSH"
read_when:
  - Setting up or debugging remote mac control
title: "Remote Control"
---

# Remote AetherBot (macOS ⇄ remote host)

This flow lets the macOS app act as a full remote control for a AetherBot gateway running on another host (desktop/server). It’s the app’s **Remote over SSH** (remote run) feature. All features—health checks, Voice Wake forwarding, and Web Chat—reuse the same remote SSH configuration from _Settings → General_.

## Modes

- **Local (this Mac)**: Everything runs on the laptop. No SSH involved.
- **Remote over SSH (default)**: AetherBot commands are executed on the remote host. The mac app opens an SSH connection with `-o BatchMode` plus your chosen identity/key and a local port-forward.
...
1. Install Node + pnpm and build/install the AetherBot CLI (`pnpm install && pnpm build && pnpm link --global`).
...
2. Under **AetherBot runs**, pick **Remote over SSH** and set:
...
3. Hit **Test remote**. Success indicates the remote `aetherbot status --json` runs correctly. Failures usually mean PATH/CLI issues; exit 127 means the CLI isn’t found remotely.
...
- Run `aetherbot channels login --verbose` **on the remote host**. Scan the QR with WhatsApp on your phone.
...
- **Health probe failed**: check SSH reachability, PATH, and that Baileys is logged in (`aetherbot status --json`).
...
```bash
aetherbot nodes notify --node <id> --title "Ping" --body "Remote gateway ready" --sound Glass
```
