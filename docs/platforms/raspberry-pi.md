---
summary: "AetherBot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up AetherBot on a Raspberry Pi
  - Running AetherBot on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# AetherBot on Raspberry Pi

## Goal

Run a persistent, always-on AetherBot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most AetherBot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled aetherbot
...
```bash
# Check logs
journalctl -u aetherbot --no-pager -n 100
