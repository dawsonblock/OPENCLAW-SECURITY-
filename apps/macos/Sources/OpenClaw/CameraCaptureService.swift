...
actor CameraCaptureService {
        ...
        delayMs: Int) async throws -> (data: Data, size: CGSize)
    {
        ...
        let rawData: Data = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Data, Error>) in
        ...
        outPath: String?) async throws -> (path: String, durationMs: Int, hasAudio: Bool)
    {
        ...
        let recordedURL: URL = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<URL, Error>) in
    ...
    private func ensureAccess(for mediaType: AVMediaType) async throws {
        ...
        case .authorized:
            return
        case .notDetermined:
            let ok = await withCheckedContinuation(isolation: nil) { (cont: CheckedContinuation<Bool, Never>) in
