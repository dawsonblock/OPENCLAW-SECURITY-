...
private fun isTrustedCanvasIpv4Host(host: String): Boolean {
  ...
  return when {
    ...
    a == 100 && b in 64..127 -> true // tailnet/CGNAT
    a in 224..239 -> true // multicast
    else -> false
  }
}

private fun isTrustedCanvasIpv6Host(host: String): Boolean {
...
private fun isTrustedCanvasHttpHost(hostRaw: String?): Boolean {
  val host = hostRaw?.trim()?.lowercase()?.trim('.') ?: return false
...
private fun isTrustedCanvasUrl(rawUrl: String?): Boolean {
  val url = rawUrl?.trim() ?: return false
  if (url.isEmpty()) return false
  if (url == "about:blank") return true

  val uri = runCatching { Uri.parse(url) }.getOrNull() ?: return false
  return when (uri.scheme?.lowercase()) {
    "http" -> isTrustedCanvasHttpHost(uri.host)
    "https" -> {
      val host = uri.host?.trim()?.lowercase()
      host == "appassets.androidplatform.net" || isTrustedCanvasHttpHost(host)
    }
    "file" -> url.startsWith("file:///android_asset/") || url.startsWith("file:///android_res/")
    else -> false
  }
}
  ...
  AndroidView(
    ...
    factory = {
      WebView(context).apply {
        ...
        fun detachBridge(view: WebView) {
          if (!bridgeAttached) return
          view.removeJavascriptInterface(CanvasA2UIActionBridge.interfaceName)
          bridgeAttached = false
        }

        fun ensureBridgeForUrl(view: WebView, url: String?): Boolean {
          if (!isTrustedCanvasUrl(url)) {
            detachBridge(view)
            return false
          }
          if (!bridgeAttached) {
            view.addJavascriptInterface(a2uiBridge, CanvasA2UIActionBridge.interfaceName)
            bridgeAttached = true
          }
          return true
        }
        ...
        webViewClient =
          object : WebViewClient() {
            override fun shouldOverrideUrlLoading(view: WebView, url: String): Boolean {
              val allowed = ensureBridgeForUrl(view, url)
              if (!allowed && isDebuggable) {
                Log.w("OpenClawWebView", "blocked navigation: $url")
              }
              return !allowed
            }
            ...
            override fun shouldOverrideUrlLoading(
              ...
              val allowed = ensureBridgeForUrl(view, nextUrl)
              if (!allowed && isDebuggable) {
                Log.w("OpenClawWebView", "blocked navigation: $nextUrl")
              }
              return !allowed
            }

            override fun onPageStarted(view: WebView, url: String?, favicon: Bitmap?) {
              if (!ensureBridgeForUrl(view, url)) {
                if (isDebuggable) {
                  Log.w("OpenClawWebView", "blocked load for untrusted origin: $url")
                }
                view.stopLoading()
                return
              }
