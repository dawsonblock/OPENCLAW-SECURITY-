...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/aetherbot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AetherBot Re-Auth" -c "After getting key, run: ssh $SERVER '~/aetherbot/scripts/mobile-reauth.sh'" --id aetherbot-auth
        ;;
    *)
