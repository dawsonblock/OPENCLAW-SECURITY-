...
IMAGE_NAME="${OPENCLAW_IMAGE:-${CLAWDBOT_IMAGE:-aetherbot:local}}"
CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${CLAWDBOT_CONFIG_DIR:-$HOME/.aetherbot}}"
WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-${CLAWDBOT_WORKSPACE_DIR:-$HOME/.aetherbot/workspace}}"
...
docker run --rm -t \
  ...
  -v "$CONFIG_DIR":/home/node/.aetherbot \
  -v "$WORKSPACE_DIR":/home/node/.aetherbot/workspace \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
