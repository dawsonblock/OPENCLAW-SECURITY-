...
#
# Suggested cron: */30 * * * * /home/admin/aetherbot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AetherBot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., aetherbot-alerts)
...
STATE_FILE="$HOME/.aetherbot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AetherBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use aetherbot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AetherBot to $NOTIFY_PHONE..."
            aetherbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AetherBot is down. Run: ssh l36 '~/aetherbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
