...
LOCK_DIR="${TMPDIR:-/tmp}/aetherbot-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${OPENCLAW_RESTART_LOG:-/tmp/aetherbot-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${OPENCLAW_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.aetherbot/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node aetherbot.mjs daemon install --force --runtime node"
...
log "==> Killing existing AetherBot instances"
kill_all_openclaw
stop_launch_agent
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node aetherbot.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node aetherbot.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const aPath = path.join(process.env.HOME, ".aetherbot", "aetherbot.json");
        const oPath = path.join(process.env.HOME, ".openclaw", "openclaw.json");
        const raw = fs.readFileSync(fs.existsSync(aPath) ? aPath : oPath, "utf8");
