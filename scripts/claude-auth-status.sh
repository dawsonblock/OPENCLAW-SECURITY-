...
# Checks both Claude Code and AetherBot auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
AETHERBOT_AUTH="$HOME/.aetherbot/agents/main/agent/auth-profiles.json"
OPENCLAW_AUTH="$HOME/.openclaw/agents/main/agent/auth-profiles.json"
if [ -f "$AETHERBOT_AUTH" ]; then
    ACTIVE_AUTH="$AETHERBOT_AUTH"
else
    ACTIVE_AUTH="$OPENCLAW_AUTH"
fi
...
fetch_models_status_json() {
    aetherbot models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    aetherbot_status=$(check_aetherbot_auth 2>/dev/null || true)

    claude_expires=0
    aetherbot_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        aetherbot_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        aetherbot_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ACTIVE_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$aetherbot_status" \
        --arg be "$aetherbot_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            aetherbot: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$aetherbot_status" == EXPIRED* ]] || [[ "$aetherbot_status" == MISSING* ]]; then
        echo "AETHERBOT_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$aetherbot_status" == EXPIRING* ]]; then
        echo "AETHERBOT_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "AetherBot Auth (~/.aetherbot/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$ACTIVE_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$ACTIVE_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'aetherbot doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
if systemctl --user is-active aetherbot >/dev/null 2>&1; then
    echo -e "AetherBot service: ${GREEN}running${NC}"
else
    echo -e "AetherBot service: ${RED}NOT running${NC}"
fi
