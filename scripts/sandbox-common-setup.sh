...
BASE_IMAGE="${BASE_IMAGE:-aetherbot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-aetherbot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=aetherbot.sandbox=1)
NOTE
