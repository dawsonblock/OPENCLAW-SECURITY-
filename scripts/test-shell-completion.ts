...
\`aetherbot update\`, \`aetherbot doctor\`, and \`aetherbot onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# AetherBot Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`aetherbot completion --install\` later to enable.`));
    return;
  }
