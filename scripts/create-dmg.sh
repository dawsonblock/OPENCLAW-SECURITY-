...
DMG_TEMP="$(mktemp -d /tmp/aetherbot-dmg.XXXXXX)"
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/aetherbot-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/aetherbot-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/aetherbot-dmg-limits.txt
