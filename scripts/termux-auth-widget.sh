...
# AetherBot Auth Widget for Termux
...
# Check auth status
termux-toast "Checking AetherBot auth..."

STATUS=$(ssh "$SERVER" '$HOME/aetherbot/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/aetherbot/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/aetherbot/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|AETHERBOT_EXPIRED|OPENCLAW_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: AetherBot won't work until re-auth"
                ;;
        esac
        ;;
