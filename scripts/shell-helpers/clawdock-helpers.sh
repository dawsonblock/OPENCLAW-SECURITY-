#!/usr/bin/env bash
# ClawDock - Docker helpers for AetherBot
# Inspired by Simon Willison's "Running AetherBot in Docker"
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/AetheronResearch/aetherbot/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for AetherBot
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/aetherbot"
  "${HOME}/workspace/aetherbot"
  "${HOME}/projects/aetherbot"
  "${HOME}/dev/aetherbot"
  "${HOME}/code/aetherbot"
  "${HOME}/src/aetherbot"
  "${HOME}/aetherbot"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found AetherBot at: $found_path"
    ...
    echo ""
    echo "❌ AetherBot not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/AetheronResearch/aetherbot.git ~/aetherbot"
    echo "  cd ~/aetherbot && ./docker-setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d aetherbot-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart aetherbot-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f aetherbot-gateway
}
...
clawdock-workspace() {
  cd ~/.aetherbot/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec aetherbot-gateway \
    bash -c 'echo "alias aetherbot=\"./aetherbot.mjs\"" > /tmp/.bashrc_aetherbot && bash --rcfile /tmp/.bashrc_aetherbot'
}
...
clawdock-cli() {
  _clawdock_compose run --rm aetherbot-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build aetherbot-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENCLAW_GATEWAY_TOKEN=$token" aetherbot-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" aetherbot-gateway \
    bash -c './aetherbot.mjs config set gateway.remote.token "$TOKEN" && ./aetherbot.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec aetherbot-gateway \
    bash -c "./aetherbot.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart aetherbot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm aetherbot-cli dashboard --no-open 2>&1)
  status=$?
...
clawdock-devices() {
  ...
  local output status
  output=$(_clawdock_compose exec aetherbot-gateway node dist/index.js devices list 2>&1)
  status=$?
  ...
  if [ $status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'aetherbot config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 AetherBot - Docker Helpers${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (aetherbot alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to aetherbot project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.aetherbot)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'aetherbot channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'aetherbot status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.aetherbot.ai${_CLR_RESET}"
  echo ""
}
