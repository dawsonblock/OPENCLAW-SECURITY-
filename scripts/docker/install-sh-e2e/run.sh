...
EXPECTED_VERSION="$(npm view "aetherbot@${INSTALL_TAG}" version)"
if [[ -z "$EXPECTED_VERSION" || "$EXPECTED_VERSION" == "undefined" || "$EXPECTED_VERSION" == "null" ]]; then
  echo "ERROR: unable to resolve aetherbot@${INSTALL_TAG} version" >&2
  exit 2
fi
...
const versions = JSON.parse(execSync("npm view aetherbot versions --json", { encoding: "utf8" }));
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  ...
  npm install -g "aetherbot@${PREVIOUS_VERSION}"
fi
...
INSTALLED_VERSION="$(aetherbot --version 2>/dev/null | head -n 1 | tr -d '\r')"
echo "installed=$INSTALLED_VERSION expected=$EXPECTED_VERSION"
if [[ "$INSTALLED_VERSION" != "$EXPECTED_VERSION" ]]; then
  echo "ERROR: expected aetherbot@$EXPECTED_VERSION, got aetherbot@$INSTALLED_VERSION" >&2
  exit 1
fi
...
set_image_model() {
  ...
  local candidate
  for candidate in "$@"; do
    if aetherbot --profile "$profile" models set-image "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
set_agent_model() {
  ...
  shift
  for candidate in "$@"; do
    if aetherbot --profile "$profile" models set "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
run_agent_turn() {
  ...
  local prompt="$3"
  local out_json="$4"
  aetherbot --profile "$profile" agent \
...
run_profile() {
  ...
  local agent_model_provider="$4" # "openai"|"anthropic"
	  ...
	  if [[ "$agent_model_provider" == "openai" ]]; then
	    aetherbot --profile "$profile" onboard \
	      --non-interactive \
	      --accept-risk \
	      --flow quickstart \
  ...
  SESSION_JSONL="/root/.aetherbot-${profile}/agents/main/sessions/${SESSION_ID}.jsonl"
  ...
  aetherbot --profile "$profile" gateway --port "$port" --bind loopback >"$GATEWAY_LOG" 2>&1 &
  GATEWAY_PID="$!"
  cleanup_profile() {
  ...
  for _ in $(seq 1 60); do
    if aetherbot --profile "$profile" health --timeout 2000 --json >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
  aetherbot --profile "$profile" health --timeout 10000 --json >/dev/null
  ...
  if [[ ! -f "$SESSION_JSONL" ]]; then
    ...
    ls -la "/root/.aetherbot-${profile}/agents/main/sessions" >&2 || true
    exit 1
  fi
...
if [[ "$MODELS_MODE" == "openai" || "$MODELS_MODE" == "both" ]]; then
  run_profile "e2e-openai" "18789" "/tmp/aetherbot-e2e-openai" "openai"
fi

if [[ "$MODELS_MODE" == "anthropic" || "$MODELS_MODE" == "both" ]]; then
  run_profile "e2e-anthropic" "18799" "/tmp/aetherbot-e2e-anthropic" "anthropic"
fi
